...
# Third-party dependencies for retrieval, generation, UI, and document parsing.
import faiss
import numpy as np
import requests
...
# App-level configuration and model defaults.
...
# Document chunk structure used by RAG indexing and retrieval.
@dataclass
class Chunk:
    text: str
    source_name: str
...
# Persistent long-term memory store helpers.
# Ensure the on-disk memory folder and JSON file exist before reads/writes.
...
# Load all saved memory items from disk; return an empty list if parsing fails.
...
# Persist the full memory item list back to disk in a stable JSON structure.
...
# Append a new timestamped memory entry to long-term storage.
...
# File parsing helpers for supported upload formats.
# Extract readable text from uploaded PDF bytes, preserving page boundaries.
...
# Extract paragraph text from uploaded DOCX bytes using a temporary file.
...
# Decode raw text bytes with common encodings and a safe fallback.
...
# Route each uploaded file to the appropriate parser based on extension.
...
# Split normalized text into overlapping chunks for embedding and retrieval.
...
# Cached model loaders to avoid reloading on Streamlit reruns.
# Load and cache the embedding model used for semantic retrieval.
@st.cache_resource(show_spinner=False)
def load_embedder(model_name: str):
    return SentenceTransformer(model_name)


# Load and cache tokenizer/model for local text generation.
@st.cache_resource(show_spinner=False)
...
# Embedding and similarity search utilities for docs and memory.
# Convert text strings into normalized float32 embedding vectors.
...
# Retrieve top-k document chunks most similar to the user query.
...
# Retrieve top-k long-term memory entries relevant to the user query.
...
# Run web search via ddgs and return title/link/snippet metadata.
...
# Fetch a webpage and return cleaned visible text truncated for prompt use.
...
# Intent detection and plan construction for tool usage.
# Detect phrases that indicate the user wants to store long-term memory.
...
# Build the tool execution plan based on question intent and runtime settings.
...
# Prompt assembly using short-term chat context and retrieved memory/context.
# Build a compact short-term context window from recent chat turns.
...
# Assemble the full model prompt from chat context, memory, and retrieved sources.
...
# Generate the assistant response from the local LLM using configured decoding.
...
# Session-state defaults for chat, plan approval, and trace display.
# Initialize required Streamlit session_state keys on first run.
def ensure_state():
...
# Core plan executor that runs approved tools and generates the final response.
# Execute planned tools, collect grounding context, and produce the final answer.
...
# Streamlit app shell and global page setup.
ensure_state()
...
# Sidebar controls for model/tool configuration and reset actions.
with st.sidebar:
...
# Main UI tabs for chat, memory management, and deployment notes.
chat_tab, memory_tab, deploy_tab = st.tabs(["Agent Chat", "Long-Term Memory", "Deploy Notes"])

# Long-term memory viewer/editor and import/export controls.
with memory_tab:
...
# Deployment instructions for local and Streamlit Cloud execution.
with deploy_tab:
...
# Chat workflow: document indexing, HITL approval, execution, and trace display.
with chat_tab:
